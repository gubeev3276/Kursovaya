...
void msgsend(int work_sock, string mess){  //Принимает два аргумента: work_sock (целое число) и mess (строка)
    char *buffer = new char[4096];  // Символьный массив
    strcpy(buffer, mess.c_str());
    send(work_sock, buffer, mess.length(), 0); // Отправка содержимого buffer
}

///@brief Метод класса для отправки сообщения
string* Server::comstr(int argc, char *argv[]) { //Принимает целое число и указатель на массив символов
    string* com = new string[4];
    if(argc == 1){
    	cout << "!!! Все параметры (-f, -p, -e,) должны быть указаны!!!\n"  << endl;
    	cout << "Введите ./main -h, чтобы получить справку.\n"  << endl;
    	string error = "Отсутствие параметров";
    ...
    while ((opt = getopt(argc, argv, "hf:p:e:")) != -1) { //Обработка аргументов командной строки
        switch(opt) {
            case 'h':
    ...
    return com; //Возвращает указатель на массив строк
}
...
Weak::MD5 hash;
    string digest;
    StringSource(sah, true,  new HashFilter(hash, new HexEncoder(new StringSink(digest))));  // строка-приемника
      return digest;
 }

///@brief Функция записи ошибок
void errors(string error, string file_error){ //Функция для записи ошибок в файл
    ofstream file;
...
int Server::self_addr(string error, string file_error, int port){
        ...
        int b = bind(s,(const sockaddr*) self_addr,sizeof(sockaddr_in)); //Привязка новосозданного сокета к адрессу
            
            
            if(b == -1) { //Если привязка не удалась
...
///@brief Метод класса для авторизации
